...
(function ($) {
  var defaults = {
    ...
    placeholderName: 'Представтесь (Ф.И.О)',
    placeholderPhone: 'Введите номер телефона',
    autoOpenForm: false,
    callFormTitle: 'Заполните форму ниже, и наш консультант свяжется с вами',
		...
		createCallForm: function () {
			...
			if (defaults.callFormAddFile) {
				...
				var fileName = $('<input>').attr({
					...
					placeholder: 'Прикрепить файл с описанием',
					disabled: 'disabled',
				});
		...
		sendMail: function (formid) {
			...
			$.ajax({
				...
				beforeSend: function () {
					...
					$(formId).find('.smartlid__respond-msgs').html('Письмо отправлено');
				},
